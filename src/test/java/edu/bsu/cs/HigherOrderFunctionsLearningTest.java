...
public class HigherOrderFunctionsLearningTest {
    ...
    public void testCountChangesByMonth() {
        Stream<Revision> input = getRevisions("soup04.json");
        Map<Month, Long> actual = input.collect(Collectors.groupingBy(this::getMonth,
                Collectors.counting()));
        Map<Month, Long> expected = Map.of(Month.FEBRUARY, 3L, Month.MARCH, 1L);
        Assertions.assertEquals(expected, actual);
    }

    private Month getMonth(Revision revision){
        return revision.timestamp.atZone(ZoneId.of("UTC"))
                .getMonth();
    }
    ...
    public void testDetermineMostActiveMonth() {
        Stream<Revision> input = getRevisions("soup30.json");
        Month actual = input.collect(Collectors.groupingBy(this::getMonth,
                Collectors.counting()))
                .entrySet()
                .stream()
